...
{
private:
	string name;//им€ склада
	size_t size;//сколько там хронитьс€ добра
	product *product_in_stok;//само добро 
public:
...
stock all_sozd()
{
	string name;
	size_t size, size1;
	cout << "¬видите им€ склада ";
	cin >> name;
	cout << " ввидите количество товара ";
	cin >> size;
	cout << endl;
	stock all(name, size);
	for (int i = 0; i < size; ++i)
	{
		cout << "¬видите им€ товара, количество, чену ";
		cin >> name >> size >> size1;
